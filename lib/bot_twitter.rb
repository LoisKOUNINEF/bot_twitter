...
def send_tweet(client)
  client.update('#bonjour_monde, @the_hacking_pro !')
end

def select_5_rdm_journalists(list)
  selection = list.sample(5)
  return selection
end

def send_tweet_journalists(selection,client)
 client.update("Love what youre doing :) #{selection} ! @the_hacking_pro #bonjour_monde !")
end
...
def perform
  ...
  list = journalists_list
  selection = select_5_rdm_journalists(list)
  send_tweet(client)
  send_tweet_journalists(selection,client)
  like_tweet(client)
  follow_tweet(client)
